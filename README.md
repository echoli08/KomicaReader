...
- 引文預覽在黑暗模式下提升編號可讀性
- 輪播播放/速度按鈕高度對齊
- 圖片載入效能優化：縮圖尺寸限制與全域 RGB_565 解碼
- 引文內容解析快取與字體大小預讀提升列表效能
...
9. **圖片輪播**：支援自動輪播與播放/暫停控制，可設定輪播秒數（含 1 秒）
10. **螢幕常亮**：可設定輪播或預覽時保持螢幕不休眠
11. **圖片操作**：長按圖片可分享或設成桌布，圖片牆長按可下載到公開相簿或分享
12. **下載路徑設定**：可在設定中切換下載到公開圖片/下載資料夾（相簿可見）
13. **響應式介面**：採用 Material Design 3 風格，支援日夜模式切換，提供流暢的閱讀體驗
14. **快取與重新整理**：支援磁碟與記憶體快取，並可透過下拉動作手動重新整理最新內容
15. **瀏覽紀錄**：自動記錄瀏覽過的討論串，支援從主頁與列表進入回顧，並提供一鍵清除功能
16. **圖片牆模式**：在討論串詳情中可切換至圖片牆模式，以網格方式快速瀏覽所有圖片
17. **回覆 WebView**：回覆改採 WebView，並可在設定中開啟「回覆頁資源精簡」加速載入
